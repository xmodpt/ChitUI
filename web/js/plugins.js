/**
 * ChitUI Plugin System - Frontend
 *
 * Handles dynamic loading and management of plugins in the UI
 */

// Plugin state
let pluginData = [];

/**
 * Load and inject plugin UI elements into the page
 */
function loadPluginUI() {
  console.log('Loading plugin UI...');

  fetch('/plugins/ui')
    .then(response => response.json())
    .then(plugins => {
      console.log('Loaded plugins:', plugins);

      plugins.forEach(plugin => {
        injectPluginUI(plugin);
      });
    })
    .catch(error => {
      console.error('Failed to load plugin UI:', error);
    });
}

/**
 * Inject a plugin's UI into the appropriate location
 */
function injectPluginUI(plugin) {
  const { type, location, html, plugin_id, title, icon } = plugin;

  if (!html) {
    console.warn(`Plugin ${plugin_id} has no HTML to inject`);
    return;
  }

  console.log(`Injecting plugin ${plugin_id} (${type}) into ${location}`);

  switch (type) {
    case 'card':
      injectCard(plugin);
      break;
    case 'toolbar':
      injectToolbar(plugin);
      break;
    case 'tab':
      injectTab(plugin);
      break;
    case 'modal':
      injectModal(plugin);
      break;
    default:
      console.warn(`Unknown plugin type: ${type}`);
  }
}

/**
 * Execute scripts embedded in plugin HTML
 * innerHTML doesn't execute scripts for security reasons, so we need to extract and execute them manually
 */
function executePluginScripts(container) {
  const scripts = container.querySelectorAll('script');
  scripts.forEach(oldScript => {
    const newScript = document.createElement('script');

    // Copy script attributes
    Array.from(oldScript.attributes).forEach(attr => {
      newScript.setAttribute(attr.name, attr.value);
    });

    // Copy script content
    newScript.textContent = oldScript.textContent;

    // Replace the old script with the new one
    oldScript.parentNode.replaceChild(newScript, oldScript);
  });
}

/**
 * Inject a plugin as a card in the main content area.
 * Plugins with location='sidebar' are placed in the dedicated sidebar column
 * next to the file manager; all others are appended to .app-content.
 */
function injectCard(plugin) {
  let container = null;

  if (plugin.location === 'sidebar') {
    container = document.getElementById('plugin-sidebar-container');
  }

  if (!container) {
    container = document.querySelector('.app-content');
  }

  if (!container) {
    console.error('Card container not found');
    return;
  }

  const pluginDiv = document.createElement('div');
  pluginDiv.id = `plugin-${plugin.plugin_id}`;
  pluginDiv.className = 'plugin-card';
  pluginDiv.innerHTML = plugin.html;

  container.appendChild(pluginDiv);

  // Execute any scripts in the plugin
  executePluginScripts(pluginDiv);
}

/**
 * Inject a plugin into the toolbar
 */
function injectToolbar(plugin) {
  // Check if this should go into the header or main content area
  if (plugin.location === 'top' || plugin.location === 'header') {
    // Inject into header toolbar (top-right)
    const headerActions = document.querySelector('.header-actions');
    if (!headerActions) {
      console.error('Header actions section not found, cannot inject toolbar');
      return;
    }

    // Create plugin container and inject HTML
    const pluginDiv = document.createElement('div');
    pluginDiv.id = `plugin-toolbar-${plugin.plugin_id}`;
    pluginDiv.className = 'plugin-header-item';
    pluginDiv.innerHTML = plugin.html;

    headerActions.appendChild(pluginDiv);

    // Extract and execute scripts (innerHTML doesn't execute scripts for security)
    executePluginScripts(pluginDiv);
  } else {
    // Inject into main content area (after printer preview)
    const printerPreview = document.querySelector('.printer-preview');
    if (!printerPreview) {
      console.error('Printer preview section not found, cannot inject toolbar');
      return;
    }

    // Create a toolbar container if it doesn't exist
    let toolbarContainer = document.getElementById('plugin-toolbar-container');
    if (!toolbarContainer) {
      toolbarContainer = document.createElement('div');
      toolbarContainer.id = 'plugin-toolbar-container';
      toolbarContainer.className = 'd-flex gap-2 mt-3';

      // Insert toolbar container after printer preview
      printerPreview.parentNode.insertBefore(toolbarContainer, printerPreview.nextSibling);
    }

    // Create plugin container and inject HTML
    const pluginDiv = document.createElement('div');
    pluginDiv.id = `plugin-toolbar-${plugin.plugin_id}`;
    pluginDiv.className = 'plugin-toolbar-item';
    pluginDiv.innerHTML = plugin.html;

    toolbarContainer.appendChild(pluginDiv);

    // Extract and execute scripts
    executePluginScripts(pluginDiv);
  }
}

/**
 * Inject a plugin as a tab
 */
function injectTab(plugin) {
  // Determine which tab container to use based on location
  let navTabsId, navPanesId;

  if (plugin.location === 'camera') {
    navTabsId = 'cameraTabs';
    navPanesId = 'cameraPanes';
  } else if (plugin.location === 'printer-info') {
    navTabsId = 'navTabs';
    navPanesId = 'navPanes';
  } else {
    // Default to printer info tabs for backward compatibility
    navTabsId = 'navTabs';
    navPanesId = 'navPanes';
  }

  // Add to navigation tabs
  const navTabs = document.getElementById(navTabsId);
  if (!navTabs) {
    console.error(`Tab container ${navTabsId} not found`);
    return;
  }

  const tab = document.createElement('li');
  tab.className = 'nav-item';
  tab.setAttribute('data-plugin-tab', plugin.plugin_id);
  tab.innerHTML = `
    <button class="nav-link" id="tab-${plugin.plugin_id}"
            data-bs-toggle="pill" data-bs-target="#tab${plugin.plugin_id}"
            type="button">
      <i class="${plugin.icon}"></i> ${plugin.title}
    </button>
  `;
  navTabs.appendChild(tab);

  // Add content pane
  const navPanes = document.getElementById(navPanesId);
  if (!navPanes) {
    console.error(`Pane container ${navPanesId} not found`);
    return;
  }

  const pane = document.createElement('div');
  pane.className = 'tab-pane fade';
  pane.id = `tab${plugin.plugin_id}`;
  pane.setAttribute('data-plugin-pane', plugin.plugin_id);
  pane.innerHTML = plugin.html;
  navPanes.appendChild(pane);

  // Execute any scripts in the plugin
  executePluginScripts(pane);
}

/**
 * Inject a plugin as a modal
 */
function injectModal(plugin) {
  const body = document.body;

  const modalDiv = document.createElement('div');
  modalDiv.id = `plugin-modal-${plugin.plugin_id}`;
  modalDiv.innerHTML = plugin.html;

  body.appendChild(modalDiv);

  // Execute any scripts in the plugin
  executePluginScripts(modalDiv);
}

/**
 * Load plugin list for management UI
 */
function loadPluginList() {
  return fetch('/plugins')
    .then(response => response.json())
    .then(data => {
      pluginData = data;
      return data;
    });
}

/**
 * Enable a plugin
 */
function enablePlugin(pluginId) {
  return fetch(`/plugins/${pluginId}/enable`, { method: 'POST' })
    .then(response => response.json())
    .then(data => {
      if (data.success) {
        console.log(`Plugin ${pluginId} enabled`);
        // Reload page to load the plugin
        window.location.reload();
      } else {
        console.error(`Failed to enable plugin: ${data.message}`);
        alert(`Failed to enable plugin: ${data.message}`);
      }
    });
}

/**
 * Disable a plugin
 */
function disablePlugin(pluginId) {
  return fetch(`/plugins/${pluginId}/disable`, { method: 'POST' })
    .then(response => response.json())
    .then(data => {
      if (data.success) {
        console.log(`Plugin ${pluginId} disabled`);
        // Reload page to unload the plugin
        window.location.reload();
      } else {
        console.error(`Failed to disable plugin: ${data.message}`);
        alert(`Failed to disable plugin: ${data.message}`);
      }
    });
}

/**
 * Delete a plugin
 */
function deletePlugin(pluginId, pluginName) {
  if (!confirm(`Are you sure you want to delete the plugin "${pluginName}"? This cannot be undone.`)) {
    return;
  }

  return fetch(`/plugins/${pluginId}/delete`, { method: 'POST' })
    .then(response => response.json())
    .then(data => {
      if (data.success) {
        console.log(`Plugin ${pluginId} deleted`);
        alert(`Plugin "${pluginName}" deleted successfully`);
        // Reload page
        window.location.reload();
      } else {
        console.error(`Failed to delete plugin: ${data.message}`);
        alert(`Failed to delete plugin: ${data.message}`);
      }
    })
    .catch(error => {
      console.error('Error deleting plugin:', error);
      alert(`Error deleting plugin: ${error}`);
    });
}

// ──────────────────────────────────────────────────────────────────────────────
// Plugin Install Wizard
// ──────────────────────────────────────────────────────────────────────────────

/** Active EventSource for install streaming (kept so it can be closed) */
let _installEventSource = null;

/**
 * Open the install wizard modal and reset it to Step 1.
 */
function openInstallWizard() {
  // Reset wizard to step 1
  document.getElementById('pluginWizardStep1').classList.remove('d-none');
  document.getElementById('pluginWizardStep2').classList.add('d-none');
  document.getElementById('pluginWizardFooter1').classList.remove('d-none');
  document.getElementById('pluginWizardFooter2').classList.add('d-none');
  document.getElementById('pluginZipInput').value = '';
  document.getElementById('pluginWizardStep1Error').classList.add('d-none');
  document.getElementById('pluginWizardStep1Error').textContent = '';
  document.getElementById('pluginInstallTerminal').innerHTML = '';
  document.getElementById('pluginInstallResult').classList.add('d-none');
  document.getElementById('pluginInstallResult').innerHTML = '';
  document.getElementById('pluginInstallSpinner').classList.remove('d-none');

  // Close any stale SSE connection
  if (_installEventSource) {
    _installEventSource.close();
    _installEventSource = null;
  }

  const modal = new bootstrap.Modal(document.getElementById('modalPluginInstall'));
  modal.show();
}

/**
 * Append a line to the terminal window.
 */
function _terminalLog(msg, level) {
  const terminal = document.getElementById('pluginInstallTerminal');
  // Remove blinking cursor if present
  const cursor = terminal.querySelector('.plugin-terminal-cursor');
  if (cursor) cursor.remove();

  const line = document.createElement('span');
  line.className = `plugin-log-line level-${level || 'info'}`;
  line.textContent = msg + '\n';
  terminal.appendChild(line);

  // Re-add blinking cursor
  const cur = document.createElement('span');
  cur.className = 'plugin-terminal-cursor';
  terminal.appendChild(cur);

  // Auto-scroll to bottom
  terminal.scrollTop = terminal.scrollHeight;
}

/**
 * Start the install: POST the zip to /plugins/upload, then open SSE stream.
 */
function startPluginInstall() {
  const fileInput = document.getElementById('pluginZipInput');
  const errorDiv = document.getElementById('pluginWizardStep1Error');

  if (!fileInput.files.length) {
    errorDiv.textContent = 'Please select a .zip file first.';
    errorDiv.classList.remove('d-none');
    return;
  }

  const file = fileInput.files[0];
  if (!file.name.endsWith('.zip')) {
    errorDiv.textContent = 'Only .zip files are accepted.';
    errorDiv.classList.remove('d-none');
    return;
  }

  // Transition to Step 2
  document.getElementById('pluginWizardStep1').classList.add('d-none');
  document.getElementById('pluginWizardStep2').classList.remove('d-none');
  document.getElementById('pluginWizardFooter1').classList.add('d-none');

  // Disable the close button while installing
  document.getElementById('btnPluginInstallClose').disabled = true;

  _terminalLog(`Uploading ${file.name}...`);

  const formData = new FormData();
  formData.append('plugin', file);

  fetch('/plugins/upload', { method: 'POST', body: formData })
    .then(r => r.json())
    .then(data => {
      if (!data.success) {
        _onInstallError(data.message || 'Upload failed.');
        return;
      }
      // Begin SSE log stream
      _streamInstallLog(data.job_id);
    })
    .catch(err => {
      _onInstallError(`Upload error: ${err}`);
    });
}

/**
 * Subscribe to the SSE install log stream for a given job_id.
 */
function _streamInstallLog(jobId) {
  const url = `/plugins/install/${jobId}/stream`;
  const es = new EventSource(url);
  _installEventSource = es;

  es.addEventListener('log', (e) => {
    try {
      const item = JSON.parse(e.data);
      _terminalLog(item.msg, item.level || 'info');
    } catch (_) {}
  });

  es.addEventListener('done', (e) => {
    es.close();
    _installEventSource = null;

    let item = {};
    try { item = JSON.parse(e.data); } catch (_) {}

    // Remove blinking cursor
    const cursor = document.getElementById('pluginInstallTerminal')
      .querySelector('.plugin-terminal-cursor');
    if (cursor) cursor.remove();

    document.getElementById('pluginInstallSpinner').classList.add('d-none');
    document.getElementById('btnPluginInstallClose').disabled = false;

    const resultDiv = document.getElementById('pluginInstallResult');
    resultDiv.classList.remove('d-none');

    if (item.success) {
      resultDiv.innerHTML = `
        <div class="alert alert-success mb-0 d-flex align-items-center gap-2">
          <i class="bi bi-check-circle-fill fs-5"></i>
          <span><strong>${item.plugin_name || 'Plugin'}</strong> installed successfully.
          Reboot the server to activate it.</span>
        </div>`;
      document.getElementById('pluginWizardFooter2').classList.remove('d-none');
    } else {
      _onInstallError(item.message || 'Installation failed.');
    }
  });

  es.onerror = () => {
    es.close();
    _installEventSource = null;
    _onInstallError('Lost connection to server during installation.');
  };
}

/**
 * Show an error state inside the wizard (Step 2).
 */
function _onInstallError(msg) {
  _terminalLog(`ERROR: ${msg}`, 'error');
  document.getElementById('pluginInstallSpinner').classList.add('d-none');
  document.getElementById('btnPluginInstallClose').disabled = false;

  const resultDiv = document.getElementById('pluginInstallResult');
  resultDiv.classList.remove('d-none');
  resultDiv.innerHTML = `
    <div class="alert alert-danger mb-0 d-flex align-items-center gap-2">
      <i class="bi bi-x-circle-fill fs-5"></i>
      <span>${msg}</span>
    </div>`;

  // Show footer with "Install Another" only
  const footer2 = document.getElementById('pluginWizardFooter2');
  footer2.classList.remove('d-none');
  document.getElementById('btnReloadAfterInstall').classList.add('d-none');
}

/**
 * Render plugin manager UI
 */
function renderPluginManager() {
  loadPluginList().then(plugins => {
    const container = document.getElementById('pluginManagerList');
    if (!container) return;

    container.innerHTML = '';

    // Install New Plugin button (opens wizard)
    const installSection = document.createElement('div');
    installSection.className = 'mb-4';
    installSection.innerHTML = `
      <button class="btn btn-primary btn-icon" id="btnOpenInstallWizard">
        <i class="bi bi-puzzle me-2"></i>Install New Plugin
      </button>
    `;
    container.appendChild(installSection);
    document.getElementById('btnOpenInstallWizard').addEventListener('click', openInstallWizard);

    if (plugins.length === 0) {
      const noPlugins = document.createElement('p');
      noPlugins.className = 'text-muted text-center py-3';
      noPlugins.textContent = 'No plugins installed yet';
      container.appendChild(noPlugins);
      return;
    }

    // Installed plugins header
    const installedHeader = document.createElement('h6');
    installedHeader.className = 'mb-3';
    installedHeader.innerHTML = '<i class="bi bi-puzzle me-2"></i>Installed Plugins';
    container.appendChild(installedHeader);

    plugins.forEach(plugin => {
      const card = document.createElement('div');
      card.className = 'card mb-3';

      const hasSettings = plugin.has_settings === true;
      const configureButton = hasSettings
        ? `<button class="btn btn-sm btn-outline-secondary" onclick="showPluginSettings('${plugin.id}')">
             <i class="bi bi-gear"></i> Configure
           </button>`
        : '';

      card.innerHTML = `
        <div class="card-body">
          <div class="d-flex justify-content-between align-items-start">
            <div class="flex-grow-1">
              <h5 class="card-title mb-1">${plugin.name}</h5>
              <p class="card-text text-muted small mb-2">${plugin.description || 'No description'}</p>
              <p class="card-text mb-0">
                <small class="text-muted">
                  Version: ${plugin.version} | Author: ${plugin.author} | ID: ${plugin.id}
                </small>
              </p>
            </div>
            <div class="d-flex align-items-center gap-2">
              ${configureButton}
              <div class="form-check form-switch">
                <input class="form-check-input" type="checkbox"
                       id="plugin-toggle-${plugin.id}"
                       ${plugin.enabled ? 'checked' : ''}
                       onchange="togglePlugin('${plugin.id}', this.checked)">
                <label class="form-check-label" for="plugin-toggle-${plugin.id}">
                  ${plugin.enabled ? 'Enabled' : 'Disabled'}
                </label>
              </div>
              <button class="btn btn-sm btn-danger" onclick="deletePlugin('${plugin.id}', '${plugin.name}')">
                <i class="bi bi-trash"></i>
              </button>
            </div>
          </div>
        </div>
      `;
      container.appendChild(card);
    });
  });
}

/**
 * Show plugin settings in a modal dialog.
 *
 * The fetched settings.html is wrapped in a div that keeps the legacy
 * id="plugin-settings-<pluginId>" so existing plugin settings pages (which
 * look up that id to hide themselves on Cancel/Close, from back when
 * settings expanded inline in the card) keep working unmodified — a
 * MutationObserver below watches for that and closes the modal instead.
 */
function showPluginSettings(pluginId) {
  const modalEl = document.getElementById('modalPluginConfig');
  const modalBody = document.getElementById('pluginConfigModalBody');
  const titleEl = document.getElementById('pluginConfigTitle');
  if (!modalEl || !modalBody) return;

  const plugin = (pluginData || []).find(p => p.id === pluginId);
  titleEl.innerHTML = `<i class="bi bi-gear me-2"></i>${plugin ? plugin.name : 'Plugin'} Settings`;
  modalBody.innerHTML = `
    <div class="text-center py-4">
      <div class="spinner-border text-secondary" role="status">
        <span class="visually-hidden">Loading...</span>
      </div>
    </div>`;

  const modal = bootstrap.Modal.getOrCreateInstance(modalEl);
  modal.show();

  fetch(`/plugin/${pluginId}/settings`)
    .then(response => response.text())
    .then(html => {
      modalBody.innerHTML = `<div id="plugin-settings-${pluginId}" class="plugin-settings-panel"></div>`;
      const panel = document.getElementById(`plugin-settings-${pluginId}`);
      panel.innerHTML = html;

      // Execute any scripts in the settings HTML
      executePluginScripts(panel);

      // Legacy Cancel/Close buttons hide the panel via style.display; treat
      // that as "close the modal" instead of leaving an empty shell open.
      const observer = new MutationObserver(() => {
        if (panel.style.display === 'none') {
          observer.disconnect();
          modal.hide();
        }
      });
      observer.observe(panel, { attributes: true, attributeFilter: ['style'] });

      modalEl.addEventListener('hidden.bs.modal', () => observer.disconnect(), { once: true });
    })
    .catch(error => {
      console.error('Error loading plugin settings:', error);
      modalBody.innerHTML = `<div class="alert alert-danger mb-0">Failed to load plugin settings.</div>`;
    });

  // Clear stale content once the modal is fully closed
  modalEl.addEventListener('hidden.bs.modal', function onHidden() {
    modalBody.innerHTML = '';
    modalEl.removeEventListener('hidden.bs.modal', onHidden);
  }, { once: true });
}

/**
 * Toggle plugin enabled/disabled
 */
function togglePlugin(pluginId, enabled) {
  if (enabled) {
    enablePlugin(pluginId);
  } else {
    disablePlugin(pluginId);
  }
}

// Initialize plugins on page load
document.addEventListener('DOMContentLoaded', () => {
  // Load plugin UI elements
  loadPluginUI();

  // If on settings page, load plugin manager
  if (document.getElementById('pluginManagerList')) {
    renderPluginManager();
  }

  // ── Plugin Install Wizard button handlers ──────────────────────────────
  const btnStart = document.getElementById('btnStartPluginInstall');
  if (btnStart) {
    btnStart.addEventListener('click', startPluginInstall);
  }

  const btnAnother = document.getElementById('btnInstallAnother');
  if (btnAnother) {
    btnAnother.addEventListener('click', () => {
      // Hide and re-show footer reload button in case it was hidden by error path
      document.getElementById('btnReloadAfterInstall').classList.remove('d-none');
      openInstallWizard();
    });
  }

  const btnReload = document.getElementById('btnReloadAfterInstall');
  if (btnReload) {
    btnReload.addEventListener('click', () => {
      btnReload.disabled = true;
      btnReload.innerHTML = '<span class="spinner-border spinner-border-sm me-1" role="status"></span>Rebooting...';
      fetch('/maintenance/restart', { method: 'POST' })
        .finally(() => {
          setTimeout(() => window.location.reload(), 5000);
        });
    });
  }

  // Close wizard SSE stream when the modal is dismissed
  const wizardModal = document.getElementById('modalPluginInstall');
  if (wizardModal) {
    wizardModal.addEventListener('hidden.bs.modal', () => {
      if (_installEventSource) {
        _installEventSource.close();
        _installEventSource = null;
      }
    });
  }
});
