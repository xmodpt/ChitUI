...
// Build marker - check the browser console to confirm which chitui.js is loaded.
// If you don't see this line, the page is running a cached copy: hard-reload with
// Ctrl+Shift+R, or open DevTools > Network and tick "Disable cache".
console.log('%c[ChitUI] chitui.js build 2026-08-24-no-reload-fix', 'color:#4ea1ff;font-weight:bold');
...
// Register UART socket events (uart.js must load before chitui.js)
$(document).ready(function () {
    if (typeof registerUartSocketEvents === 'function') {
        registerUartSocketEvents(socket);
    }
});
...
function tryAutoSelectDefaultPrinter() {
  if (!currentPrinter && defaultPrinterLoaded && printersLoaded && defaultPrinterId && printers[defaultPrinterId]) {
    ...
    // On mobile the sidebar is hidden by default, but the content area may not
    // be in view yet. Scroll to top and ensure the sidebar overlay is not blocking.
    if (window.innerWidth <= 992) {
      const _sidebar = document.querySelector('.app-sidebar');
      const _overlay = document.getElementById('sidebarOverlay');
      if (_sidebar) {
        _sidebar.classList.remove('show');
      }
      if (_overlay) {
        _overlay.classList.remove('show');
      }
      document.body.style.overflow = '';
      // Scroll to top so the printer dashboard is the first thing seen
 ...
 *
 * NOTE: this used to call location.reload() whenever any printer flipped
 * online <-> offline. A single missed websocket pong on the printer side was
 * therefore enough to reload the whole page (twice: once going down, once
 * coming back), even though the browser <-> Pi connection was perfectly fine.
 * We now refresh the affected parts of the UI in place instead.
 *
...
socket.on("printers", (data) => {
  ...
  // Detect online <-> offline transitions (for targeted refreshes, not reloads)
  var cameOnline = []
  for (var id in data) {
    ...
    printersPreviousStatus[id] = currentOnline
  }

  // Forget printers the server no longer reports
  for (var oldId in printersPreviousStatus) {
    if (!(oldId in data)) {
      delete printersPreviousStatus[oldId]
      delete printers[oldId]
    }
  }

  // Merge the server-side fields into our local objects instead of replacing
  // them wholesale: status / attributes / files / printTracking are
  // client-side caches and must survive a printers update, now that we no
  // longer reload the page to rebuild them.
  for (var id in data) {
    printers[id] = Object.assign(printers[id] || {}, data[id])
  }

  // Re-render the sidebar list (online badges included)
  $("#printersList").empty()
  addPrinters(printers)
  refreshPrinterListStatus()

  // Mark printers as loaded and try to auto-select default
  printersLoaded = true

  if (currentPrinter && printers[currentPrinter]) {
    // Selected printer reconnected: pull fresh info instead of reloading
    if (cameOnline.indexOf(currentPrinter) !== -1) {
      console.log('Selected printer is back online - refreshing its data')
      socket.emit("printer_info", { id: currentPrinter })
      lastFileCount = 0
...
/**
 * Re-apply cached per-printer status text/icons to the sidebar after the list
 * has been re-rendered, so entries don't fall back to the template's "Idle"
 * until the next status packet arrives.
 */
function refreshPrinterListStatus() {
  $.each(printers, function (id, p) {
    if (p && p.status && p.status.CurrentStatus !== undefined) {
      var cs = p.status.CurrentStatus
      updatePrinterStatus({
        MainboardID: id,
        Status: { CurrentStatus: Array.isArray(cs) ? cs : [cs] }
      })
    }
  })
}
...
function handle_printer_status(data) {
  ...
  // Only update the Status/Print tab UI if this event is for the currently selected printer
  if (data.MainboardID === currentPrinter) {
...
function handle_printer_attributes(data) {
  ...
  // Only update the Attributes tab UI if this event is for the currently selected printer
  if (data.MainboardID !== currentPrinter) {
    return;
  }

  // Add printer serial number (MainboardID) to attributes display
  var attributesWithSerial = { 'SerialNumber': data.MainboardID }
  $.each(data.Attributes, function (key, val) {
    attributesWithSerial[key] = val
  })
  createTable('Attributes', attributesWithSerial)
...
function handle_printer_files(data) {
  ...
  // Only update the Files UI if this response is for the currently selected printer
...
function addPrinters(printers) {
  $.each(printers, function (id, printer) {
    ...
    // Show UART badge in sidebar for UART printers
    if (printer.connection_type === 'uart') {
      item.find('.printerType').after(
        '<span class="badge bg-warning text-dark ms-1" style="font-size:0.62rem;vertical-align:middle;">' +
        '<i class="bi bi-usb-symbol"></i> UART</span>'
      );
    }
    ...
    // Only poll printers that are actually reachable. The list is re-rendered on
    // every 'printers' event now (no page reload), so asking an offline printer
    // for status/attributes here would just add pointless SDCP traffic.
    if (printer.online === true) {
...
function showPrinter(id) {
  ...
  // Clear only printer-specific tabs (Status, Print, Attributes, Files).
  // Plugin tabs (e.g. rpi_stats) are system-level and must be preserved.
  $('#navTabs li:not([data-plugin-tab])').remove();
  $('#navPanes .tab-pane:not([data-plugin-pane])').remove();

  // On mobile, close the sidebar overlay so the printer view is immediately visible
  const _sidebar = document.querySelector('.app-sidebar');
  const _overlay = document.getElementById('sidebarOverlay');
  if (_sidebar && window.innerWidth <= 992) {
    _sidebar.classList.remove('show');
    if (_overlay) _overlay.classList.remove('show');
    document.body.style.overflow = '';
  }

  // UART integration: show/hide controls, disable local tab
  if (typeof onShowPrinter === 'function') {
    onShowPrinter(id, p);
  }
  ...
  // Show UART badge in preview header
  $('#printerUartBadge').remove();
  if (p.connection_type === 'uart') {
    $('#printerType').after(
      '<span id="printerUartBadge" class="badge bg-warning text-dark ms-2" style="font-size:0.75rem;">' +
      '<i class="bi bi-usb-symbol me-1"></i>UART</span>'
    );
  }
  ...
  if (p.status) {
    ...
    // No status available for this printer (e.g. UART) — show a simple message tab
    // Create the tab manually so we can render a plain message instead of a card grid
    if ($('#tab-Status').length == 0) {
      var tTab = $("#tmplNavTab").html()
      var tab = $(tTab)
      tab.find('button').attr('id', 'tab-Status').attr('data-bs-target', '#tabStatus').text('Status').addClass('active')
      $('#navTabs').append(tab)
    }
    if ($('#tabStatus').length == 0) {
      var tPane = $("#tmplNavPane").html()
      var pane = $(tPane)
      pane.attr('id', 'tabStatus').addClass('active show')
      pane.find('tbody').attr('id', 'tableStatus').closest('table').replaceWith(
        '<p class="text-muted p-3 mb-0">Status is not available for this connection type.</p>'
      )
      $('#navPanes').append(pane)
    }
  }
  if (p.attributes && Object.keys(p.attributes).length > 0) {
    var cachedAttrsWithSerial = { 'SerialNumber': id }
    $.each(p.attributes, function (key, val) {
      cachedAttrsWithSerial[key] = val
    })
    createTable('Attributes', cachedAttrsWithSerial)
    // Restore storage gauge from cached attributes when switching back to this printer
    if (p.attributes.RemainingMemory !== undefined) {
      updateStorageDisplay(p.attributes.RemainingMemory);
    }
  } else {
    // No attributes available for this printer (e.g. UART) — clear the table and show N/A
    var naAttrs = {
      'SerialNumber': id,
      'Name': p.name || 'N/A',
      'Machine Name': p.model || 'N/A',
      'Brand Name': p.brand || 'N/A',
      'Connection Type': p.connection_type ? p.connection_type.toUpperCase() : 'N/A',
      'Resolution': 'N/A',
      'X Y Zsize': 'N/A',
      'Firmware Version': 'N/A',
      'Protocol Version': 'N/A'
    }
    createTable('Attributes', naAttrs)
    // Hide storage gauge for printers that don't report storage (e.g. UART)
    $('#storageInfo').hide();
  }

  // Always reset lastFileCount so the file manager re-renders when switching printers
  lastFileCount = 0;

  // Clear the file manager UI immediately to avoid showing stale files
  const fileManagerBodySwitch = document.getElementById('fileManagerBody');
  if (fileManagerBodySwitch) {
    fileManagerBodySwitch.innerHTML = '<tr><td colspan="4" class="text-center text-muted py-4"><i class="bi bi-hourglass-split" style="font-size: 2rem; display: block; margin-bottom: 0.5rem;"></i>Loading files...</td></tr>';
  }
  if ($('#tableFiles').length > 0) {
    $('#tableFiles').empty();
  }

  // If we have cached files for this printer, show them immediately
  if (printers[id]['files'] !== undefined && printers[id]['files'].length > 0) {
    console.log('Displaying cached files for printer:', id, '(', printers[id]['files'].length, 'files)');
    createTable('Files', printers[id]['files'])
    addFileOptions()
  }
  // Always fetch a fresh file list when switching printers
  console.log('Requesting fresh file list for printer:', id);
  ...
  // UART: if a print is in progress (tracked via uart_status), restore the overlay
  if (p.connection_type === 'uart' && p.uart_status && p.uart_status.machine_status === 1) {
    var uartFilename = p.uart_status.Filename || p._uartFilename || '';
    updatePrintOverlay(id, {
      Status:       3,
      CurrentLayer: p.uart_status.CurrentLayer || 0,
      TotalLayer:   p.uart_status.TotalLayer   || 0,
      Filename:     uartFilename,
      CurrentTicks: 0,
      TotalTicks:   0,
      ErrorNumber:  0
    });
...
function createTable(name, data, active = false) {
  ...
  // The File Manager mirrors #tableFiles. Sync it right after the source table
...
function getPrinterFiles(id, url) {
  // UART printers have no WebSocket — use the dedicated REST endpoint instead
  var printer = printers[id];
  if (printer && printer.connection_type === 'uart') {
    // Only fetch once per call (the REST endpoint returns all files in one shot)
    // Guard against the double-call pattern getPrinterFiles(id,'/local') + (id,'/usb')
    if (url !== '/usb') return;  // skip the /local call, handle everything on /usb
    fetch('/uart/files/' + id)
      .then(function(r) { return r.json(); })
      .then(function(data) {
        if (!data.success) {
          console.warn('UART file list error:', data.message);
          return;
        }
        populateUartFileManager(id, data.files);
      })
      .catch(function(e) { console.error('UART file fetch error:', e); });
    return;
  }
...
/**
 * Render the file manager directly from the UART /uart/files REST response.
 * Each file object has: { name, path, size, thumb_small, thumb_big }
 */
function populateUartFileManager(printerId, files) {
  console.log('populateUartFileManager: ' + files.length + ' files for ' + printerId);

  // Keep the legacy printers[id]['files'] array in sync so other code that
  // checks printers[id]['files'].length still works correctly.
  var paths = files.map(function(f) { return f.path; });
  printers[printerId] = printers[printerId] || {};
  printers[printerId]['files'] = paths;

  // Also update the hidden #tableFiles so addFileOptions() keeps working
  var t = $('#tableFiles');
  t.empty();
  $.each(paths, function(i, p) {
    t.append('<tr data-file="' + p + '"><td class="fieldKey">' + i + '</td><td class="fieldValue">' + p + '</td></tr>');
  });
  addFileOptions();

  // Now render the rich file manager
  var fileManagerBody = document.getElementById('fileManagerBody');
  if (!fileManagerBody) return;

  lastFileCount = 0;  // force syncTableToFileManager to accept the update

  if (files.length === 0) {
    fileManagerBody.innerHTML =
      '<tr><td colspan="4" class="text-center text-muted py-4">' +
      '<i class="bi bi-inbox" style="font-size:2rem;display:block;margin-bottom:0.5rem;"></i>' +
      'No files on USB. Upload a file to get started.</td></tr>';
    return;
  }
  ...
  files.forEach(function(f, index) {
    ...
    thumbCell.style.cssText = 'width:100px;text-align:center;';
    if (f.thumb_big) {
      var img = document.createElement('img');
      img.src = f.thumb_big;
      img.style.cssText = 'width:80px;height:80px;object-fit:contain;border-radius:6px;border:2px solid #3a3a3a;background:#1a1a1a;cursor:pointer;';
      img.alt = 'Preview';
      img.addEventListener('click', function() { openThumbnailLightbox(this.src); });
      img.onerror = function() {
        this.style.display = 'none';
        var ph = document.createElement('div');
        ph.textContent = '📄';
        ph.style.cssText = 'font-size:2rem;width:80px;height:80px;display:flex;align-items:center;justify-content:center;color:#555;';
    ...
    idxCell.style.width = '50px';
    row.appendChild(idxCell);

    // Filename cell
    var nameCell = document.createElement('td');
    nameCell.textContent = f.path;
    ...
    var printBtn = document.createElement('i');
    printBtn.className = 'bi bi-printer-fill fileOption ps-1';
    printBtn.title = 'Print this file';
    printBtn.style.cursor = 'pointer';
    printBtn.addEventListener('click', function() {
      console.log('[print] Sending action_print for printer:', currentPrinter, 'file:', filePath);
      socket.emit('action_print', { id: currentPrinter, data: filePath });
    });

    var delBtn = document.createElement('i');
    delBtn.className = 'bi bi-trash-fill fileOption ps-2';
    delBtn.title = 'Delete file';
    delBtn.style.cursor = 'pointer';
    delBtn.addEventListener('click', function() {
      if (confirm('Delete ' + f.name + '?')) {
        socket.emit('action_delete', { id: currentPrinter, data: filePath });
      }
    });
...
function addFileOptions() {
  $('#tableFiles .fieldValue').each(function () {
    // Guard: this runs again on every file-list refresh. Without the check we
    // append a second (third, fourth...) set of icons to the same cell, and
    // every one of them fires - which is why a single download click showed up
    // as three identical requests in the log.
    if ($(this).find('.fileOption').length) return

    var file = $(this).text()
    var options = $('<i class="bi bi-printer-fill fileOption ps-3" data-action="print" data-file="' + file + '" data-bs-toggle="tooltip" title="Print this file"></i><i class="bi bi-download fileOption ps-1" data-action="download" data-file="' + file + '" data-bs-toggle="tooltip" title="Download file"></i><i class="bi bi-trash-fill fileOption ps-1" data-action="delete" data-file="' + file + '" data-bs-toggle="tooltip" title="Delete file"></i>')
    $(this).append(options)
    $(this).parent().attr('data-file', file)
  })
  // Scope the binding to #tableFiles ONLY.
  //
  // The icons the user actually clicks live in #fileManagerBody: syncTableToFileManager()
  // clones them out of this hidden table and attaches its own native addEventListener
  // handler to each clone. A bare $('.fileOption') selector also matches those clones,
  // so every later call to addFileOptions() bolted a second, jQuery handler onto icons
  // that were already wired up - one click, two downloadFile() calls.
  //
  // cloneNode(true) does not copy jQuery's handler cache, so restricting the selector
  // here leaves the clones with exactly one handler: their own.
  $('#tableFiles .fileOption').off('click').on('click', function (e) {
    var action = $(this).data('action')
    var file = $(this).data('file')

    // Handle download action directly without confirmation
    if (action === 'download') {
      downloadFile(file)
      return
    }
    ...
    if (action === 'print' && typeof window.timelapsePrintHook === 'function') {
      window.timelapsePrintHook(file, function (withTimelapse) {
        ...
        console.log('[print] Sending action_print for printer:', currentPrinter, 'file:', file);
...
function updatePrintOverlay(printerId, printInfo) {
  ...
  } else if (!printInfo.TaskId && !tracking.thumbnailRequested && printInfo.Filename) {
    // UART printer: no TaskId, but we know the filename — load thumbnail directly from disk
    tracking.thumbnailRequested = true;
    var stem = printInfo.Filename.replace(/\.[^.]+$/, '');
    var thumbUrl = '/thumbnails/' + stem + '_big.png';
    var testImg = new Image();
    testImg.onload = function() {
      if (!printers[printerId]) return;
      if (!printers[printerId].currentPrint) printers[printerId].currentPrint = {};
      printers[printerId].currentPrint.thumbnail = thumbUrl;
      if (currentPrinter === printerId) displayThumbnail(thumbUrl);
    };
    testImg.onerror = function() {
      console.log('No thumbnail found for UART file:', stem);
    };
  ...
  if (printInfo.Status !== SDCP_PRINT_STATUS_IDLE &&
    ...
    if (!$('#modalPrintStatus').hasClass('show') && !tracking.modalShownOnce) {
      ...
      }, 800); // 800ms: enough for any preceding modal to finish its hide animation
    }
  } else {
    // Hide main card when print is complete/idle.
    // For UART printers: if _uartFilename is still set the printer is just
    // silent (Mars 2 stops responding during print) — don't tear down yet.
    var uartStillPrinting = printers[printerId] &&
                            printers[printerId].connection_type === 'uart' &&
                            printers[printerId]._uartFilename;
    if (!uartStillPrinting) {
...
$('#btnStartCamera').on('click', function() {
  ...
  $.ajax({
    ...
    data: JSON.stringify({ printer_id: currentPrinter }),
...
$('#btnFullscreenCamera').on('click', function() {
  ...
  // The Pi Cam tab shares this modal, so set the title and sizing explicitly
  // rather than relying on whatever the markup or the last opener left behind.
  $('#modalCameraFullscreen .modal-title')
    .html('<i class="bi bi-camera-video me-2"></i>Camera Stream');
...
/**
 * Rotate a file's cached thumbnail and remember the choice server-side.
 * Cache-busts the <img> tags afterwards, otherwise the browser keeps showing
 * the old orientation from its own cache even though the PNG on disk changed.
 */
function rotateThumbnail(filename, transform) {
  var base = filename.replace(/^\/(local|usb)\//, '');

  $.ajax({
    url: '/thumbnails/orient',
    method: 'POST',
    contentType: 'application/json',
    data: JSON.stringify({ filename: base, transform: transform || 'rot90' }),
    success: function (d) {
      if (!d.success) {
        showToast(d.message || 'Could not rotate thumbnail', 'warning');
        return;
      }
      ...
      $('img').each(function () {
        ...
        if (src.indexOf(stem + '_small.png') !== -1 || src.indexOf(stem + '_big.png') !== -1) {
          $(this).attr('src', src.split('?')[0] + bust);
        }
      });
      showToast('Thumbnail rotated', 'success');
    },
    error: function (xhr) {
      showToast((xhr.responseJSON || {}).message || 'Could not rotate thumbnail', 'danger');
    }
  });
}
...
// Tracks the last download we kicked off, so a double-fire from any call site
// can't turn one click into two hits. This matters beyond the wasted bandwidth:
// Chrome only treats the FIRST download in a gesture as user-initiated, and
// files with unrecognised extensions (.goo, .ctb) are held to an
// "allow on user gesture" policy. The second download of the same click looks
// automatic to Chrome, which is what raises the Keep/Discard warning.
var _lastDownload = { name: null, at: 0 };

function downloadFile(filename) {
  /**
   * Download a file from the printer's storage
   * Constructs download URL and triggers browser download
   */
  if (!currentPrinter) {
  ...
  var printer = printers[currentPrinter];
  if (!printer) {
  ...
  var now = Date.now();
  if (_lastDownload.name === filename && (now - _lastDownload.at) < 2000) {
    console.log('Ignoring duplicate download request for:', filename);
    return;
  }
  _lastDownload = { name: filename, at: now };

  // Printer file listings report paths like "/local/model.goo" or
  // "/usb/model.goo" (the storage location baked into the path). Strip
  // that prefix before building the URL: encodeURIComponent() turns the
  // "/" into "%2F", and Flask's default routing decodes %2F back into a
  // literal slash *before* matching routes, so a single-segment "/download/
  // <printer_id>/<filename>" route no longer matches and the request 404s
  // (shows up in the browser as "File wasn't available on site"). Passing
  // just the bare filename avoids the slash entirely.
  var cleanFilename = filename;
  if (cleanFilename.indexOf('/local/') === 0) {
    cleanFilename = cleanFilename.slice('/local/'.length);
  } else if (cleanFilename.indexOf('/usb/') === 0) {
    cleanFilename = cleanFilename.slice('/usb/'.length);
  }

  // Construct the download URL
  var downloadUrl = '/download/' + currentPrinter + '/' + encodeURIComponent(cleanFilename);

  console.log('Downloading file:', filename, '->', cleanFilename, 'from printer:', currentPrinter);

  // Navigate rather than synthesising an <a download> click.
  //
  // Two reasons. First, if something still manages to call this twice in the
  // same tick, the browser collapses both into a single navigation - two
  // anchor clicks would start two independent downloads. Second, Chrome treats
  // extensions it doesn't recognise (.goo, .ctb) as "allow on user gesture",
  // and a plain navigation from inside a click handler is the cleanest way to
  // keep that gesture attached. The server sets Content-Disposition, so the
  // page doesn't actually navigate away.
...
$(document).ready(function () {
  ...
  // When the confirm modal fully closes, check if a UART print just started
  // and is waiting to show the print status modal. Bootstrap blocks two modals
  // from being open simultaneously, so we defer the print modal until here.
  $('#modalConfirm').on('hidden.bs.modal', function () {
    if (!currentPrinter) return;
    var tracking = printers[currentPrinter] && printers[currentPrinter].printTracking;
    if (tracking && tracking.modalShownOnce && !$('#modalPrintStatus').hasClass('show')) {
      if (printStatusModal) {
        printStatusModal.show();
      }
    }
...
$('#btnCleanThumbnails').on('click', function() {
  if (!confirm('This will delete cached thumbnails that are no longer used by any existing print file.\n\nThumbnails for files that still exist will NOT be deleted.\n\nContinue?')) {
    return;
  }
  ...
  fetch('/thumbnails/clean', { method: 'POST' })
    .then(function(r) { return r.json(); })
    .then(function(data) {
      $btn.prop('disabled', false).html(originalText);
      if (data.success) {
        const res = data.results;
        const freedKb = (res.freed_bytes / 1024).toFixed(1);
        const msg = 'Deleted ' + res.deleted + ' orphaned thumbnail(s), kept ' + res.kept +
                    ' in use, pruned ' + res.db_pruned + ' stale record(s). Freed ' + freedKb + ' KB.';
        $('#thumbCleanResult').text(msg).show();
        alert('Thumbnail cache cleaned!\n\n' + msg);
      } else {
        alert('Failed to clean thumbnails: ' + (data.message || 'Unknown error'));
      }
    })
    .catch(function(err) {
      $btn.prop('disabled', false).html(originalText);
      console.error('Thumbnail clean error:', err);
      alert('Error cleaning thumbnail cache');
    });
});
...
// Respect the saved color mode (set by the header light/dark switcher).
// Users who never picked a mode keep the classic dark default.
...
if (mobileMenuToggle) {
  mobileMenuToggle.addEventListener('click', function() {
    // On mobile, always allow toggling sidebar even if pinned
    if (sidebar.classList.contains('pinned')) {
      // Toggle the show class to open/close sidebar on mobile
      sidebar.classList.toggle('show');
      sidebarOverlay.classList.toggle('show');
      document.body.style.overflow = sidebar.classList.contains('show') ? 'hidden' : '';
    } else {
...
if (sidebarOverlay) {
  sidebarOverlay.addEventListener('click', function() {
    // Close sidebar when clicking overlay, even if pinned (for mobile)
    if (sidebar.classList.contains('pinned')) {
...
// File manager sync
//
// This used to run from a MutationObserver on document.body (subtree: true)
// plus a 2 second interval. Every printer status packet rebuilds #tableStatus
// (createTable -> fillTable -> t.empty()), so the observer fired roughly once
// per second forever, re-scanning the table and spamming the console, even
// though the file list had not changed.
//
// The file list only changes when handle_printer_files() renders it, so the
// sync is now driven from createTable('Files', ...) instead of watching the
// whole page for mutations.
// ---------------------------------------------------------------------------

var FILE_SYNC_DEBUG = false;   // set true to restore the old verbose logging
...
function hideFilesTab() {
  ...
  const el = filesTab.parentElement;
  // Only write the styles if they aren't already applied - re-writing them on
  // every call produced pointless attribute mutations.
...
// lastFileCount is kept because several call sites set it to 0 to force a
// re-render (printer switch, upload, delete, refresh). It now acts purely as
// that "force" flag; the real change detection uses a content signature, so a
// rename or a swapped file with the same count is no longer ignored.
let lastFileCount = 0;
var lastFileSignature = null;

function syncTableToFileManager(sourceTbody) {
  const fileManagerBody = document.getElementById('fileManagerBody');
  if (!fileManagerBody) {
    if (FILE_SYNC_DEBUG) console.warn('fileManagerBody not found!');
    return;
  }
  if (!sourceTbody) {
    if (FILE_SYNC_DEBUG) console.warn('sourceTbody not provided!');
    return;
  }
  ...
  let signature = '';
  rows.forEach(r => {
    signature += (r.getAttribute('data-file') || r.textContent) + '|';
  });

  if (lastFileCount !== 0 && signature === lastFileSignature) {
    if (FILE_SYNC_DEBUG) console.log('File list unchanged, skipping sync');
    return;
  }

  lastFileSignature = signature;
  lastFileCount = rows.length;
  if (FILE_SYNC_DEBUG) console.log(`Syncing ${rows.length} files to File Manager`);
  ...
  rows.forEach((row, index) => {
    ...
    if (cells.length >= 2) {
      ...
      const filenameWithPath = tempDiv.childNodes[0]?.textContent || '';

      // Extract just the filename without path prefix (/local/, /usb/, etc.)
      ...
      // Extract file extension to check if it supports thumbnails
      const ext = filename.split('.').pop().toLowerCase();
      if (ext === 'goo' || ext === 'ctb') {
        const stem = filename.substring(0, filename.lastIndexOf('.'));
        const thumbnailImg = document.createElement('img');
        thumbnailImg.src = `/thumbnails/${stem}_big.png`;
        ...
        thumbnailImg.style.border = '2px solid #3a3a3a';
        thumbnailImg.style.background = '#1a1a1a';
        thumbnailImg.style.cursor = 'pointer';
        thumbnailImg.alt = 'Preview';

        // Add click handler for lightbox
        thumbnailImg.addEventListener('click', function() {
          openThumbnailLightbox(this.src);
        });
      ...
      filenameCell.textContent = filenameWithPath; // Show full path in table
      ...
      expandBtn.innerHTML = '<i class="bi bi-chevron-double-down"></i>';
      expandBtn.setAttribute('aria-label', 'Show file details');
      ...
      // Rotate-thumbnail button. Deliberately manual: nothing can reliably work
      // out which way up a 3D render belongs, so the user decides. Each click is
      // 90 degrees clockwise; four clicks return to the original.
      const rotateBtn = document.createElement('i');
      rotateBtn.className = 'bi bi-arrow-clockwise fileOption ps-1';
      rotateBtn.setAttribute('title', 'Rotate thumbnail 90\u00B0');
      rotateBtn.style.cursor = 'pointer';
      rotateBtn.addEventListener('click', function (e) {
        e.stopPropagation();
        rotateThumbnail(filenameWithPath, 'rot90');
      });
      ...
      // Create expandable details row for mobile
      const detailsRow = document.createElement('tr');
      detailsRow.className = 'file-details-row';
      const detailsCell = document.createElement('td');
      detailsCell.colSpan = 4;

      const detailsContent = document.createElement('div');
      detailsContent.className = 'file-details-content';

      // File name detail
      const nameItem = document.createElement('div');
      nameItem.className = 'file-detail-item';
      nameItem.innerHTML = `
        <span class="file-detail-label">File Name:</span>
        <span class="file-detail-value">${filename}</span>
      `;
      detailsContent.appendChild(nameItem);

      // Full path detail
      const pathItem = document.createElement('div');
      pathItem.className = 'file-detail-item';
      pathItem.innerHTML = `
        <span class="file-detail-label">Path:</span>
        <span class="file-detail-value">${filenameWithPath}</span>
      `;
      detailsContent.appendChild(pathItem);

      // File number detail
      const numberItem = document.createElement('div');
      numberItem.className = 'file-detail-item';
      numberItem.innerHTML = `
        <span class="file-detail-label">File #:</span>
        <span class="file-detail-value">${index + 1}</span>
      `;
      detailsContent.appendChild(numberItem);

      detailsCell.appendChild(detailsContent);
      detailsRow.appendChild(detailsCell);
      fileManagerBody.appendChild(detailsRow);
    }
  });

  console.log('File Manager updated with', fileManagerBody.children.length, 'files');

  // Initialize Bootstrap tooltips for file manager icons
  var tooltipTriggerList = [].slice.call(document.querySelectorAll('#fileManagerTable [data-bs-toggle="tooltip"]'))
  tooltipTriggerList.map(function (tooltipTriggerEl) {
    return new bootstrap.Tooltip(tooltipTriggerEl)
  })
}
...
// Safety net for anything that writes #tableFiles without going through
// createTable(). Was 2000ms; the sync is event-driven now, so this is just a
// slow backstop.
setInterval(syncFilesToFileManager, 30000);
...
// Thumbnail lightbox
function openThumbnailLightbox(imageSrc) {
  // Create lightbox if it doesn't exist
  let lightbox = document.getElementById('thumbnailLightbox');
  if (!lightbox) {
    lightbox = document.createElement('div');
    lightbox.id = 'thumbnailLightbox';
