...
# Security settings
# NOTE: NoNewPrivileges is intentionally NOT set here. ChitUI escalates to
# root only through narrow NOPASSWD sudoers rules for specific helper
# scripts (Tailscale, USB gadget, Settings -> Network's network_helper.sh).
# NoNewPrivileges blocks the setuid bit that sudo relies on entirely, which
# breaks all of those features even with a correct sudoers rule in place.
PrivateTmp=true
