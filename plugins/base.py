...
class ChitUIPlugin(ABC):
    ...
    def install_dependencies(self):
        ...
        if deps:
            # Use sys.executable -m pip to avoid broken pip3 shebang issues
            pip_cmd = [sys.executable, '-m', 'pip']
            for dep in deps:
                try:
                    # Try with --break-system-packages first (for Debian 12+)
                    subprocess.check_call(pip_cmd + ['install', dep, '--break-system-packages'])
                except subprocess.CalledProcessError:
                    try:
                        # Fallback to --user if --break-system-packages fails
                        subprocess.check_call(pip_cmd + ['install', '--user', dep])
                    except subprocess.CalledProcessError:
                        return False
        return True
