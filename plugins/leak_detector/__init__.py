...
def _ensure_package(import_name, apt_package=None, pip_package=None):
    """
    Check if a Python package is importable; if not, attempt to install it.
    Tries apt-get first (preferred on Raspberry Pi OS), then falls back to pip.
    ...
    pip_name = pip_package or import_name
    logger.info(f"Package '{import_name}' not found - attempting auto-install...")

    # 1. Try apt-get (works best on Raspberry Pi OS, no --user issues)
    if apt_package:
        try:
            ...
            if result.returncode == 0:
                logger.info(f"Auto-installed '{import_name}' via apt ({apt_package})")
                try:
                    __import__(import_name)
                    return True
                except ImportError:
                ...
                logger.debug(f"apt install {apt_package} failed (rc={result.returncode}): {result.stderr.strip()}")
        except Exception as e:
            logger.debug(f"apt install attempt failed: {e}")

    # 2. Fall back to pip with various strategies for system-managed Python envs
    pip_attempts = [
    ...
    for cmd in pip_attempts:
        try:
            ...
            if result.returncode == 0:
                logger.info(f"Auto-installed '{import_name}' via pip")
                try:
                    __import__(import_name)
                    return True
                except ImportError:
            ...
            logger.debug(f"pip install attempt {cmd} failed: {e}")

    logger.warning(
        f"Could not auto-install '{import_name}'. "
        f"Please install manually: sudo apt install {apt_package or pip_name}"
    )
    return False


# Ensure optional dependencies are present before importing them
_ensure_package('requests', apt_package='python3-requests', pip_package='requests')
_ensure_package('zeroconf', apt_package='python3-zeroconf', pip_package='zeroconf')
...
# mDNS (zeroconf) for bidirectional discovery with ESP32
try:
    ...
    logger.info("zeroconf not available - ESP32 mDNS auto-discovery disabled. Install with: sudo apt install python3-zeroconf")
...
class Plugin(ChitUIPlugin):
    ...
    def __init__(self, plugin_dir):
        ...
        # mDNS (zeroconf) for bidirectional discovery
        self.zeroconf = None
        self.mdns_service = None
        self._esp32_browser = None
    ...
    def get_notification_definitions(self):
        """Define notification types for leak detector events"""
        return {
            'leak_detected': {
                'name': 'LEAK DETECTED',
                'enabled': True,
    ...
    def _do_send_notification(self, alarm_id, extra_message=None):
        """Send notification if enabled in plugin config"""
        config_key = f'notify_{alarm_id}'
        if not self.config.get(config_key, False):
            return  # Notification not enabled in this plugin's config

        self.send_notification(alarm_id, extra_message)
    ...
    def arm_relay(self, reason=None):
        ...
        if self._set_relay(False):  # OFF = cut power
            ...
            self._do_send_notification('leak_relay_armed', f"Reason: {reason}")
    ...
    def disarm_relay(self, user=None):
        ...
        if self._set_relay(True):  # ON = restore power
            ...
            # Send push notification if enabled
            self._do_send_notification('leak_relay_disarmed', f"Disarmed by: {user or 'user'}")
    ...
    def on_startup(self, app, socketio):
        ...
        def reset_detection():
            ...
            try:
                ...
                # Send push notification if enabled
                self._do_send_notification('leak_reset', 'Leak detection system has been reset. Monitoring resumed.')
        ...
        # Register ChitUI via mDNS so ESP32 can discover it, and browse for ESP32
        self._start_mdns(app)
    ...
    def _process_esp32_sensor_data(self, data, esp_ip):
        ...
        for key, sensor_info in data.items():
            ...
            if (is_leak and is_confirmed) and not prev_alert:
                ...
                self._do_send_notification('leak_detected', f"{sensor_name} at {sensor_location} - Value: {value}")
    ...
    def _start_mdns(self, app):
        """
        Register ChitUI as _chitui._tcp.local. so the ESP32 firmware can find it
        via MDNS.queryService("chitui", "tcp"), and browse _http._tcp.local. to
        auto-discover the ESP32 leak sensor (advertised as leak-sensor._http._tcp.local.).
        """
        if not ZEROCONF_AVAILABLE:
            logger.warning("zeroconf not installed - ESP32 mDNS auto-discovery disabled. "
        ...
        try:
            ...
            # Determine the local LAN IP by connecting a UDP socket (no data sent)
            ...
            self.mdns_service = ServiceInfo(
                "_chitui._tcp.local.",
                "ChitUI._chitui._tcp.local.",
                addresses=[_socket.inet_aton(local_ip)],
                port=port,
                properties={"version": "2.1", "name": "ChitUI"},
            )
            self.zeroconf.register_service(self.mdns_service)
            logger.info(f"mDNS: Registered ChitUI as _chitui._tcp.local. at {local_ip}:{port}")

            # Browse for an already-online ESP32 (registered as _http._tcp.local.)
            self._browse_for_esp32()

        except Exception as e:
            logger.error(f"mDNS setup failed: {e}")

    def _browse_for_esp32(self):
        """Browse _http._tcp.local. and capture the ESP32's IP when it appears."""
        if not self.zeroconf:
            return

        plugin_self = self

        class _ESP32Listener:
            def add_service(self, zc, type_, name):
                if "leak-sensor" not in name.lower() and "leak_sensor" not in name.lower():
                    return
                info = zc.get_service_info(type_, name)
                if info and info.addresses:
                    ip = _socket.inet_ntoa(info.addresses[0])
                    logger.info(f"mDNS: Discovered ESP32 leak sensor at {ip}:{info.port}")
                    # Only auto-configure if we don't already have an IP
                    if not plugin_self.device_status.get('ip'):
                        plugin_self.device_status['ip'] = ip
                        logger.info(f"mDNS: Auto-configured ESP32 IP to {ip}")
        ...
        self._esp32_browser = ServiceBrowser(self.zeroconf, "_http._tcp.local.", _ESP32Listener())
        logger.info("mDNS: Browsing for ESP32 leak sensor (_http._tcp.local.)")

    def _stop_mdns(self):
        """Unregister ChitUI mDNS service and close zeroconf."""
        if self.zeroconf:
            try:
    ...
    def _register_esp32_endpoints(self, app):
        ...
        def leak_alert():
            ...
            try:
                ...
                self._do_send_notification('leak_detected', f"{sensor_name} at {sensor_location} - Value: {data.get('value')}")
